...
$pesquisar = mysqli_query($conn, "SELECT * FROM usuarios where login = '".$login."' ");

$contagem = mysqli_num_rows($pesquisar);
//echo $contagem;
if ( $contagem == 1 ) {  $errors = "- O nome de login que você escolheu já está cadastrado.<br>";  }

if ( $senha != $senha2 ) {  $errors = "- As duas senhas não correspondem.<br>";  }
//  echo $errors;
if ( $errors == "" ) {
  $senha = md5($senha);
  $cadastrar = mysqli_query($conn,"insert into usuarios(nome, responsavel, endereco, bairro, cel, cadastur, login, senha, ativado, email, nivel_usuario,dtcad) VALUES ('".$nome."','".$responsavel."','".$endereco."','".$bairro."','".$cel."','".$cadastur."','".$login."','".$senha."','".$ativado."','".$email."','".$nivel."','".$dtcad."')");
...
?>
  <form action="area_administrador.php?nome=<?php echo $usuario ?>" method="post" name="" id="">
